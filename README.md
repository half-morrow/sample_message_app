...
## Pre-commit hook
devbox で lefthook を提供し、pre-commit 時の lint / format は Docker Compose 経由で実行します。初回は Docker image と依存 volume を準備してから hook を有効化してください。
...
hook は staged file のみを対象にします。

部分 stage している file は formatter / fixer により同一 file 内の未 stage 行も書き換えられ、再 stage される場合があります。必要に応じて commit 前に `git diff --cached` で確認してください。
...
## テスト・ビルド
### Lint / format
pre-commit hook では staged file に限定して Docker Compose 経由で RuboCop safe autocorrect、ESLint fix、Prettier write を実行します。手動で全体確認する場合は以下を使います。
